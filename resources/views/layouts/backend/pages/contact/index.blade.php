<x-backend.layout>
    ...
    <div class="p-3 xl:p-6">
        <div class="flex justify-between items-center mt-1 mb-6">
            <p class="text-xl font-bold uppercase">quan ly lien he</p>
            <div>
                <a href="{{ route('contact.create') }}" class="rounded-lg py-1 px-3 bg-[#059655] font-semibold capitalize text-white"><i class="fa-solid fa-plus mr-1"></i>Them lien he</a>
                <a href="{{ route('contact.trash') }}" class="rounded-lg py-1 px-3 bg-red-500 font-semibold capitalize text-white"><i class="fa-solid fa-trash mr-1"></i>Thung rac</a>
            </div>
        </div>
        ...
        <form method="GET" action="">
            <div class="flex gap-3">
                ...
                <a href="{{ route('contact.index') }}" class="rounded-lg py-1 px-3 bg-white self-center px-4 py-2">Reset</a>
            </div>
        </form>
        ...
        <table class="border-collapse bg-white rounded-lg border border-pink-500 overflow-hidden mt-3 mx-auto w-[99%]">
            <thead>
                <tr>
                    ...
                    <th class="py-1 px-3 text-center">Title</th>
                    <th class="py-1 px-3 text-center">Replay ID</th>
            ...
            <tbody>
                @foreach ($contacts as $contact)
                    <tr class="border-t-3 border-gray-200 hover:bg-gray-50 text-center">
                        <td class="py-3 px-1">{{ $contact->id }}</td>
                        <td class="py-3 px-1">{{ $contact->name }}</td>
                        <td class="py-3 px-1">{{ $contact->email }}</td>
                        <td class="py-3 px-1">{{ $contact->phone }}</td>
                        <td class="py-3 px-1">{{ $contact->title }}</td>
                        <td class="py-3 px-1">{{ $contact->replay_id }}</td>
                        <td class="py-3 px-1">{{ $contact->status }}</td>
                        <td class="py-3 px-1 align-middle">
                            <div class="flex flex-nowrap gap-2">
                                <div class="rounded-lg shadow text-sm p-3 hover:bg-gray-100">
                                    <a href="{{ route('contact.edit', $contact->id) }}"><i class="fa-solid fa-pen"></i><span class="hidden ml-1 xl:inline">Edit</span></a>
                                </div>
