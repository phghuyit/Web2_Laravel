    ...
    <div class="p-3 xl:p-6">
        ...
        <div class="flex items-center justify-between mb-6 mt-1">
            ...
            <div>
                <a href="{{ route('product.create') }}"
                    class="bg-[#059655] capitalize font-semibold px-3 py-1 rounded-lg text-white"><i
                        class="fa-plus fa-solid mr-1"></i>Thêm sách</a>
                <a href="{{ route('product.trash') }}"
                    class="bg-red-500 capitalize font-semibold px-3 py-1 rounded-lg text-white"><i
                        class="fa-solid fa-trash mr-1"></i>Thùng rác</a>
            </div>
        ...
        <form method="GET" action="">
            <div class="flex gap-3">
                ...
                <div class="flex-1">
                    <select name="brand_id" class="bg-white border px-4 py-2 rounded-lg w-full"
                        onchange="this.form.submit()">
                        <option value="" class="text-sm">Tác giả: All</option>
                        @foreach ($brands as $brand)
                            <option value="{{ $brand->id }}" class="text-sm"
                                {{ request('brand_id') == $brand->id ? 'selected' : '' }}>{{ $brand->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex-1">
                    <select name="cat_id" class="bg-white border flex-1 px-4 py-2 rounded-lg w-full"
                        onchange="this.form.submit()">
                        <option value="">Thể loại: All</option>
                        @foreach ($cats as $cat)
                            <option value="{{ $cat->id }}" class="text-sm"
                                {{ request('cat_id') == $cat->id ? 'selected' : '' }}>{{ $cat->name }}
                                {{ request('cat_id') == $cat->id ? 'selected' : '' }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex-1">
                    <select name="sort_by" class="bg-white border flex-1 px-4 py-2 rounded-lg w-full"
                        ...
                        <option value="price_desc" {{ request('sort_by') == 'price_desc' ? 'selected' : ' ' }}>Giá giảm dần
                        </option>
                    </select>
                </div>
                    ...
                    <tbody>
                        @foreach ($products as $product)
                            <tr class="border-gray-200 border-t text-center hover:bg-gray-50">
                                ...
                                <td class="px-1 py-3">
                                    <span
                                        class="px-6 py-3 rounded-full font-semibold {{ $product->status == 1 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
