    ...
    xl:p-6">
        ...
        <form method="GET" action="">
            <div class="flex gap-3">
                ...
                <div class="flex-1">
                    <select name="brand_id" class="border bg-white px-4 py-2 rounded-lg w-full">
                        <option value="" class="text-sm">Tác giả: All</option>
                            @foreach ($brands as $brand)
                                <option value="{{ $brand->id }}" class="text-sm">{{ $brand->name }}</option>
                            @endforeach
                    </select>
                </div>
                <div class="flex-1">
                    <select name="cat_id" class="border bg-white px-4 py-2 rounded-lg flex-1 w-full">
                        <option value="" >Thể loại: All</option>
                        @foreach ($cats as $cat)
                            <option value="{{ $cat->id }}" class="text-sm">{{ $cat->name }}</option>
                        @endforeach
                    </select>
                </div>
